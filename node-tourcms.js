...
TourCMS.prototype.makeRequest = function (a) {
  ...
  // Sensible defaults
  if (typeof a.channelId == "undefined") a.channelId = 0;

  if (typeof a.verb == "undefined") a.verb = "GET";
  ...
  var req = https.request(options, function (response) {
    ...
    function apiResponded(apiResponse) {
      ...
      parser.parseString(apiResponse, function (err, result) {
        ...
        else a.callback(result.response);
      });
    }
  });

  // If we're posting, let's post
  if (a.verb == "POST") req.write(apiParams);
...
TourCMS.prototype.apiRateLimitStatus = function (a) {
  ...
  if (typeof a.channelId === "undefined") a.channelId = this.options.channelId;
...
TourCMS.prototype.searchTours = function (a) {
  ...
  // Set API path
  if (parseInt(a.channelId) === 0) a.path = "/p/tours/search.xml?" + a.qs;
  else a.path = "/c/tours/search.xml?" + a.qs;
...
TourCMS.prototype.listTours = function (a) {
  ...
  // Set API path
  if (a.channelId === 0) a.path = "/p/tours/list.xml?" + a.qs;
  else a.path = "/c/tours/list.xml?" + a.qs;
...
TourCMS.prototype.showTourDatesDeals = function (a) {
  ...
  // Add in the TourId in if provided separately
  if (typeof a.tourId !== "undefined") a.qs.id = a.tourId;
  ...
  a.processor = function (response, callback) {
    ...
    if (typeof response.dates_and_prices === "undefined")
      ...
      response.dates_and_prices.date = [].concat(
        response.dates_and_prices.date
      );
...
TourCMS.prototype.getDeparturesOverview = function (a) {
  ...
  if (typeof a.date !== "undefined") params.date = this.toTourcmsDate(a.date);

  if (typeof a.productTypes !== "undefined")
    params.product_type = a.productTypes.join(",");

  if (typeof a.page !== "undefined") params.page = a.page;

  if (typeof a.perPage !== "undefined") params.per_page = a.perPage;

  a.path =
    "/c/tour/datesprices/dep/manage/overview.xml?" +
  ...
  a.processor = function (response, callback) {
    ...
    if (typeof response.tour === "undefined") response.tour = [];
    else response.tour = [].concat(response.tour);

    // Check each tour has an array of departures
    response.tour.forEach(function (tour) {
      if (tour.departures === "") tour.departures = { departure: [] };
      else tour.departures.departure = [tour.departures.departure].concat();
    });
...
TourCMS.prototype.showDeparture = function (a) {
  ...
  a.path =
    "/c/tour/datesprices/dep/manage/show.xml?id=" +
    a.tourId +
    "&departure_id=" +
    a.departureId;
  ...
  a.processor = function (response, callback) {
    if (typeof response.tour.bookings === "undefined")
      ...
      response.tour.bookings.booking = [].concat(
        response.tour.bookings.booking
      );
...
TourCMS.prototype.listBookings = function (a) {
  const q = `made_date_start=${a.date}`;
  let path;
...
TourCMS.prototype.checkTourAvailability = function (a) {
  ...
  a.processor = function (response, callback) {
    if (!response.available_components)
      ...
      response.available_components.component = [].concat(
        response.available_components.component
      );
    ...
    response.available_components.component.forEach(function (component) {
      // Price rows
      component.price_breakdown.price_row = [].concat(
        component.price_breakdown.price_row
      );
      ...
      if (typeof component.pickup_points === "undefined")
        ...
        component.pickup_points.pickup = [].concat(
          component.pickup_points.pickup
        );
    });
...
TourCMS.prototype.searchBookings = function (a) {
  ...
  // Set API path
  if (parseInt(a.channelId) === 0) a.path = "/p/bookings/search.xml?" + a.qs;
  else a.path = "/c/bookings/search.xml?" + a.qs;
...
TourCMS.prototype.startNewBooking = function (a) {
  ...
  if (typeof a.booking === "undefined") a.booking = {};
...
TourCMS.prototype.searchVouchers = function (a) {
  ...
  if (typeof a.voucherString === "undefined") a.voucherString = "";
  ...
  // Set API path
  if (parseInt(a.channelId) === 0) a.path = "/p/voucher/search.xml";
  else a.path = "/c/voucher/search.xml";
...
TourCMS.prototype.redeemVoucher = function (a) {
  ...
  if (typeof a.note === "undefined") a.note = "";
...
TourCMS.prototype.createPayment = function (a) {
  ...
  if (typeof a.payment === "undefined") a.payment = {};
...
TourCMS.prototype.listPayments = function (a) {
  ...
  a.processor = function (response, callback) {
    ...
    // Ensure we have an array of tours
    else response.payment = [].concat(response.payment);
...
TourCMS.prototype.listStaffMembers = function (a) {
  ...
  a.processor = function (response, callback) {
    // Ensure we have a total tour count
    if (typeof response.total_users === "undefined") response.total_users = "0";
    else response.users.user = [].concat(response.users.user);
  ...
  a.processor = function (response, callback) {
    if (response.error == "OK" && response.customer) {
      ...
      if (typeof response.customer.custom_fields !== "undefined")
        response.customer.custom_fields.field = [].concat(
          response.customer.custom_fields.field
        );
      else response.customer.custom_fields = { field: [] };
    }
...
TourCMS.prototype.createEnquiry = function (a) {
  ...
  if (typeof a.enquiry === "undefined") a.enquiry = {};
...
TourCMS.prototype.searchEnquiries = function (a) {
  ...
  // Set API path
  if (parseInt(a.channelId) === 0) a.path = "/p/enquiries/search.xml?" + a.qs;
  else a.path = "/c/enquiries/search.xml?" + a.qs;
...
TourCMS.prototype.updateCustomer = function (a) {
  ...
  // Enquiry object, create empty one if it doesn't exist
  if (typeof a.customer === "undefined") a.customer = {};
...
TourCMS.prototype.generateSignature = function (
  path,
  channelId,
  verb,
  outboundTime,
